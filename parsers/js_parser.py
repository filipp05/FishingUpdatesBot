    ...
    try:
        ...
        for card in product_cards:
            ...
            price = price_elem.text.strip().replace(
                ' ', '').replace('₽', '') if price_elem else None

            # Старая цена и скидка
            old_price_elem = card.find('span', class_='oldprice')
            old_price = old_price_elem.text.strip().replace(
                ' ', '').replace('₽', '') if old_price_elem else None
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if temp_timestamp - prev_timestamp >= TIME_PERIOD:
